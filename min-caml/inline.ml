...
let rec is_simple_recursive name body =
  match body with
  | IfEq(_,_,e1,e2)
  | IfLE(_,_,e1,e2)
  | Let (_,  e1,e2) -> (is_simple_recursive name e1) || (is_simple_recursive name e2)
  | LetTuple(_,_,e)
  | LetRec(_,e)     ->  is_simple_recursive name e
  | App(g,_)        -> (g == name)
  | _               -> false
...
let rec g env = function (* インライン展開ルーチン本体 (caml2html: inline_g) *)
  ...
  | LetRec({ name = (x, t); args = yts; body = e1 }, e2) ->
      let env =
        if is_simple_recursive x e1 then
          env
        else
          M.add x (yts, e1) env
      in
